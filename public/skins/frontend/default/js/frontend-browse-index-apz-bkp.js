...
function sortResetData(resp,type,page,func){
    ...
    $("#tBody_table_td tr").remove()
           $.each(data,function(s,item){
            if(item['countTraks'] == null){
                var traks = '0 Plays'
            }else{
                traks = item['countTraks']+' Plays'
            }
            if(item['count_vote'] == null){
                var count_vote = '0 Votes'
            }else{
                count_vote = item['count_vote']+' Votes'
            }
            if(item['countFanOfBand'] == null){
                var count_fans = '0 Fans'
            }else{
                count_fans = item['countFanOfBand']+' Fans'
            }
            $("#tBody_table_td").append(
                    '<tr>'+
                        '<td><span style="float:left; margin:0 10px 0 0;"><img src="'+$projectExecImage+'/'+item['vImg']+'" alt="album" height="52px" width="52px"></span>'+
                            '<h2>'+item['vTitle']+'</h2>'+
                            '<h4 style="color:#666;">Los Angeles, CA</h4>'+
                        '</td>'+
                        ...
                        '<td class="maine-data1"><a href="'+$projectExecPath+'/profile/band/id/'+item['iBand_id']+'" class="viewprofilebtn">VIEW PROFILE</a></td>'+
                    '</tr>'+
