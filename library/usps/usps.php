    ...
    function getPrice() {
        ...
        if(count($array['ERROR'])) { // If it is error
            ...
            foreach ($array['RATEV2RESPONSE'][0]['PACKAGE'][0]['POSTAGE'] as $value){
                $price = new price();
                $price->mailservice = str_replace('&lt;sup&gt;&amp;reg;&lt;/sup&gt;','',$value['MAILSERVICE'][0]['VALUE']);
            ...
            foreach($array['INTLRATERESPONSE'][0]['PACKAGE'][0]['SERVICE'] as $value) {
				...
				$price->svcdescription = str_replace('&lt;sup&gt;&amp;reg;&lt;/sup&gt;','',$value['SVCDESCRIPTION'][0]['VALUE']);
				$price->svcdescription = str_replace('**','',$price->svcdescription);
