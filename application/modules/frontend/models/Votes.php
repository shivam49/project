...
class Frontend_Model_Votes extends Zend_Db_Table_Abstract {
...
//        }
 ...
 public function getTotalRecord($whereCondition='') {
   ...
   public function getList($whereCondition='', $start='', $limit='', $sort='', $dir='',$array = '*') {
    ...
    public function getCountFansVote($whereCondition='') {
        $objSelect = $this->select()
                            ->distinct()
                            ->from($this->_name,'iMember_id')
                            ->where(prepareCondition($whereCondition));

        $rowset = $objSelect->query()->fetchAll();
